...
public class Updater {
    ...
    private void downloadFile() {
        ...
        try {
            ...
            int count;
            if (this.announce) {
            	Bukkit.getConsoleSender().sendMessage("§aAbout to download a new update:§e " + this.versionName);
            }
            long downloaded = 0;
            while ((count = in.read(data, 0, Updater.BYTE_SIZE)) != -1) {
                ...
                if (this.announce && ((percent % 10) == 0)) {
                    Bukkit.getConsoleSender().sendMessage("§4Downloading update:§a " + percent + "% §4of§a " + fileLength + " §4bytes.");
                }
            }
        } catch (Exception ex) {
    ...
    public boolean shouldUpdate(String localVersion, String remoteVersion) {
    	return (getFileVersion(localVersion)) < (getFileVersion(remoteVersion));
